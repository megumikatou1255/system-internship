...
## Luồng hoạt động của các thành phần trong Neutron
- **Tiếp nhận yêu cầu (neutron-server)**:
    + Người dùng gửi yêu cầu tạo mạng hoặc tạo cổng qua CLI/Horizon tới neutron-server.
    + neutron-server xác thực token qua Keystone, kiểm tra tính hợp lệ và ghi nhận cấu hình vào Neutron database.

- **Đẩy thông điệp qua hàng đợi (Queue)**:
    + Sau khi cập nhật cơ sở dữ liệu, neutron-server phát đi một thông điệp thông báo thay đổi cấu hình vào Queue.

- **Phối hợp xử lý từ các Agent**:
    + Các agent (neutron-l2-agent, neutron-l3-agent, neutron-dhcp-agent) đang lắng nghe thông điệp từ Queue sẽ nhận được tín hiệu.
    + Các agent này đối chiếu với Neutron database để lấy chi tiết cấu hình mới nhất.

- **Hiện thực hóa hạ tầng mạng**:
    + Các agent tiến hành cấu hình thực tế lên tầng ảo hóa của hệ thống (ví dụ: tạo bridge, thiết lập namespace, cấu hình iptables, cấp phát IP qua DHCP cho máy ảo).

## Các thành phần sẽ được cài đặt trên Node nào
- Cài đặt trên Controller Node:
    + neutron-server
    + Neutron database
    + Queue (Message Queue)
- Cài đặt trên Network Node (hoặc Controller Node trong mô hình All-in-One):
    + neutron-l3-agent (quản lý định tuyến và Floating IP)
    + neutron-dhcp-agent (cấp phát IP)
- Cài đặt trên Compute Node (và Network Node):
    + Neutron L2 agent (chạy trực tiếp trên từng Compute Node để quản lý kết nối mạng L2 cho các máy ảo đang chạy trên node đó).