...
- **Infra**
+ SQL Database (ví dụ: MariaDB / MySQL): Nơi lưu trữ toàn bộ cơ sở dữ liệu dùng chung cho tất cả các service (Nova DB, Neutron DB, Glance DB, Keystone DB,...).
+ Message Queue (ví dụ: RabbitMQ): Hệ thống hàng đợi thông điệp, đóng vai trò là "trạm trung chuyển" tín hiệu bất đồng bộ giữa các service và các agent.
+ NTP (Network Time Protocol): Dịch vụ đồng bộ thời gian hệ thống. Điều này cực kỳ quan trọng để đảm bảo log và các token xác thực không bị lệch thời gian giữa các node.

- **Openstack**
Đây là các tiến trình quản lý trung tâm của các service chính:
+ Identity Service (Keystone): Cung cấp dịch vụ định danh, xác thực token và quản lý quyền hạn cho mọi người dùng và service.
+ Image Service (Glance): Quản lý metadata và định danh của các tệp ảnh máy ảo (glance-api).
+ Compute Management (Nova): Chạy các thành phần quản lý điều phối như nova-api, nova-scheduler, nova-conductor.
+ Networking Management (Neutron): Chạy tiến trình máy chủ mạng neutron-server và các trình điều khiển plugin.
+ Dashboard (Horizon): Giao diện Web quản lý trực quan cho người quản trị và người dùng cuối.
