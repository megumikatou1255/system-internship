...
## Luồng hoạt động của các thành phần trong Nova
- **Tiếp nhận yêu cầu (nova-api)**:
    + Người dùng gửi yêu cầu tạo máy ảo qua CLI/Horizon tới nova-api. nova-api xác thực token qua Keystone, kiểm tra hạn mức và ghi bản ghi ban đầu vào Nova database.

- **Chuyển tiếp qua hàng đợi (Queue)**:
    + nova-api đẩy thông điệp yêu cầu vào Queue để chuyển đến bộ lập lịch.

- **Lựa chọn máy chủ (nova-scheduler)**:
    + nova-scheduler lấy thông điệp từ hàng đợi, phối hợp với cơ sở dữ liệu/Placement để chọn ra compute node phù hợp nhất, sau đó gửi tiếp thông điệp qua Queue.

- **Xử lý trung gian an toàn (nova-conductor)**:
    + Thông điệp được chuyển qua nova-conductor. Thành phần này thay mặt compute node thực hiện các truy vấn đọc/ghi cần thiết xuống Nova database nhằm đảm bảo bảo mật.

- **Thi hành tạo máy ảo (nova-compute & Hypervisor)**:
    + Tiến trình nova-compute trên compute node nhận lệnh từ Queue, sau đó tương tác trực tiếp với tầng Hypervisor (như KVM) để khởi tạo tệp cấu hình, cấp phát tài nguyên và bật máy ảo (Instance).

## Các thành phần của Nova được cài đặt trên Node nào
- Cài đặt trên Controller Node:
    + nova-api
    + nova-scheduler
    + nova-conductor
    + nova-console, nova-consoleauth, nova-cert
    + Nova database và Queue (Message Queue)
- Cài đặt trên Compute Node:
    + nova-compute (chạy trực tiếp trên từng máy chủ vật lý quản lý máy ảo)
    + Hypervisor (tầng ảo hóa phần cứng của node đó)